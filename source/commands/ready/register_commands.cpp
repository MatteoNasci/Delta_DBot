...
mln::register_commands::register_commands(dpp::cluster& cluster, cmd_runner& in_runner_cmd_ptr, cmd_ctx_runner& in_runner_ctx_ptr) noexcept : base_ready{ cluster }, runner_cmd_ptr{ in_runner_cmd_ptr }, runner_ctx_ptr{ in_runner_ctx_ptr } {
    cbot().log(dpp::loglevel::ll_debug, std::format("register_commands: [{}].", true));
}

dpp::task<void> mln::register_commands::command(const dpp::ready_t& event_data) {
    if (dpp::run_once<struct register_bot_commands>()) {
        cbot().log(dpp::loglevel::ll_debug, "The bot will clean and re-register the commands!");
        ...
        for (size_t i = 0; i < cmds.size(); ++i) {
            for (size_t j = i + 1; j < cmds.size(); ++j) {
                if (mln::utility::is_same_cmd(cmds[i], cmds[j])) {
                    cbot().log(dpp::loglevel::ll_error, std::format("Failed to register commands, found duplicate local commands! Command name: [{}], indexes: [{}, {}].", 
                        cmds[i].name, i, j));
                    co_return;
                }
            }
        ...
        if (get_commands_res.is_error()) {
            ...
            cbot().log(dpp::loglevel::ll_error, std::format("Failed to get registered commands from API! Error: [{}], details: [{}].\nProceeding with commands registration...", 
            ...
            cbot().log(dpp::loglevel::ll_error, "Failed to get registered commands from API! The confirmation_callback doesn't have a valid map. Proceeding with commands registration...");
        } 
        else{
        ...
        if (!mismatch) {
            cbot().log(dpp::loglevel::ll_debug, "Found no mismatch between local commands and registered global command. Commands registration skipped!");

            co_return;
        }

        cbot().log(dpp::loglevel::ll_debug, "Found mismatch between local commands and registered global command. Proceeding with commands registration...");

        const dpp::confirmation_callback_t register_res = co_await bot().co_global_bulk_command_create(cmds);
        if (!register_res.is_error()) {
            cbot().log(dpp::ll_debug, "Updated global commands!");
        }
        else {
            const dpp::error_info err = register_res.get_error();
            cbot().log(dpp::loglevel::ll_error, std::format("Failed to register new commands! Error: [{}], details: [{}].", mln::get_json_err_text(err.code), err.human_readable));
...
std::optional<std::function<void()>> mln::register_commands::job(const dpp::ready_t& event_data)
{
    cbot().log(dpp::loglevel::ll_critical, "Failed to register commands! Usage of job command instead of task command.");
    return std::nullopt;
}

bool mln::register_commands::use_job() const noexcept
{
    return false;
}
