...
mln::insert_guild_db::insert_guild_db(bot_delta* const in_delta) : base_guild_create(delta), saved_insert_guild_query(), delta(in_delta) {
    ...
    valid_saved_insert = (res == mln::db_result::ok);
}
...
dpp::job mln::insert_guild_db::command(std::shared_ptr<dpp::guild_create_t> event) {
    ...
    if (res != mln::db_result::ok) {
        delta->bot.log(dpp::loglevel::ll_error, "Failed to bind guild " + std::to_string(guild_id) + " to insert guild stmt! " + mln::database_handler::get_name_from_result(res));
        co_return;
    }
    
    res = delta->db.exec(saved_insert_guild_query, mln::database_callbacks_t());
    if (res != mln::db_result::ok) {
        delta->bot.log(dpp::loglevel::ll_error, "Failed to execute insert guild " + std::to_string(guild_id) + " stmt! " + mln::database_handler::get_name_from_result(res));
    }
}