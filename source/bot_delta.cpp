...
void mln::bot_delta::setup_db() {
    ...
    res = database.save_statement("SELECT * FROM guild_profile; SELECT * FROM storage ORDER BY guild_id ASC, user_id ASC, creation_time ASC; SELECT * FROM report ORDER BY creation_time ASC;", saved_select_all_query);
    if (res.type != mln::db_result::ok) {
